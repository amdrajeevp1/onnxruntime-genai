...
std::unique_ptr<NamedTensors> Qwen3VLImageProcessor::Process(const Tokenizer& tokenizer, const Payload& payload) const {
  ...
  // Expect CHW format: [channels, height, width]
  ...
  int64_t channels = pixel_target_shape[0];
  int64_t height = pixel_target_shape[1];
  int64_t width = pixel_target_shape[2];
  ...
  auto patch_tensor = ConvertToPatch(static_cast<const float*>(pixel_data), 
