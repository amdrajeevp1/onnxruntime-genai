...
class Qwen3VLVisionRotaryEmbedding(nn.Module):
    ...
    def __init__(self, dim: int, theta: float = 10000.0, max_seqlen: int = 1024) -> None:
        super().__init__()
        self.dim = dim
        self.theta = theta
        ...
        # Precompute frequency table for ONNX export (static shape)
    ...
    def forward(self, seqlen: int) -> torch.Tensor:
        # For ONNX export compatibility, use precomputed table
        if seqlen <= self.max_seqlen:
            return self.freq_table[:seqlen]
        else:
            # Fallback for sequences longer than max (won't happen in ONNX)
...
class Qwen3VLVisionModel(Qwen3VLPreTrainedModel):
    ...
    def rot_pos_emb(self, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        # For ONNX export: use fixed max size to avoid data-dependent shapes
        # Assume max image size of 1024x1024 (can be adjusted based on typical input)
        max_hw = 1024
        freq_table = self.rotary_pos_emb(max_hw)  # (max_hw, dim // 2)
        device = freq_table.device

        # Calculate total tokens with static operations
        # For typical case: grid_thw is [num_images, 3] where each row is [t, h, w]
        batch_size = grid_thw.shape[0]
        
        # Preallocate with max possible size
        max_tokens_per_image = (max_hw // merge_size) ** 2  # max merged tokens per image
        max_total_tokens = batch_size * max_tokens_per_image * 16  # with some buffer for frames
        pos_ids = torch.zeros((max_total_tokens, 2), dtype=torch.long, device=device)
        ...
        for idx in range(batch_size):
            num_frames = grid_thw[idx, 0].item()
            height = grid_thw[idx, 1].item()
            width = grid_thw[idx, 2].item()
            ...
            offset += num_tokens
            actual_total_tokens += num_tokens

        # Trim to actual size
        pos_ids = pos_ids[:actual_total_tokens]
    ...
    def fast_pos_embed_interpolate(self, grid_thw):
        ...
        for idx in range(grid_thw.shape[0]):
            ...
            # Use explicit steps for ONNX compatibility instead of linspace with dynamic size
            h_step = (self.num_grid_per_side - 1) / (h - 1) if h > 1 else 0
            w_step = (self.num_grid_per_side - 1) / (w - 1) if w > 1 else 0
            
            h_idxs = torch.arange(h, device=device, dtype=torch.float) * h_step
            w_idxs = torch.arange(w, device=device, dtype=torch.float) * w_step
