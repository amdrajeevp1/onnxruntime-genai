...
def build_vision(args):
    ...
    image_1 = Image.open(requests.get(url, stream=True, verify=False).raw)
    url = "https://img.freepik.com/free-photo/painting-mountain-lake-with-mountain-background_188544-9126.jpg?w=2000"
    image_2 = Image.open(requests.get(url, stream=True, verify=False).raw)
    url = "https://th.bing.com/th/id/OIP.gCvQ1vmPVJmrq1nnzM3ZHQHaEo?rs=1&pid=ImgDetMain"
    image_3 = Image.open(requests.get(url, stream=True, verify=False).raw)
    url = "https://wallpaper.dog/large/10809054.jpg"
    image_4 = Image.open(requests.get(url, stream=True, verify=False).raw)
    ...
    torch.onnx.export(
        ...
        dynamo=False,  # Use TorchScript export instead of torch.export for better compatibility
    )
...
def build_embedding(args):
    ...
    torch.onnx.export(
        ...
        opset_version=14,
        do_constant_folding=True,
        dynamo=False,  # Use TorchScript export for better compatibility
    )
